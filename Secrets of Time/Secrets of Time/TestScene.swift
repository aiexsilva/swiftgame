...
class TestScene: SKScene, SKPhysicsContactDelegate {
    ...
    private func setupCamera() {
        ...
        cam.setScale(TestScene.cameraZoom)
        addChild(cam)
        camera = cam
    }
    ...
    private func makeButton(label: String, position: CGPoint) -> SKShapeNode {
        let node = SKShapeNode(circleOfRadius: TestScene.buttonRadius)
    ...
    private func hudButtonHit(at point: CGPoint) -> String? {
        ...
        for case let button? in buttons {
            ...
            if (dx * dx + dy * dy) <= TestScene.buttonRadius * TestScene.buttonRadius {
                return button.name
            }
        }
        return nil
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if let cam = camera {
            let halfVisW = visibleSize.width * TestScene.cameraZoom / 2
            let halfVisH = visibleSize.height * TestScene.cameraZoom / 2
    ...
    private func triggerPortalTransition() {
        ...
        overlay.onComplete = { [weak self] in
            ...
            let next = TestScene(size: currentSize)
